...
export class DashboardItemValueComponent implements OnInit, OnDestroy, WidgetElement {
  ...
  public set Config(value: WidgetConfig | undefined) {
    ...
    DataRight = 0;
    destroy$ = new Subject();
    numericFields: DashboardDataSourceField[] = [];
    ...
    getMainField() {
      if (this.DataSource) {
        const ret = this.DataSource.Fields.find(v=>v.Property==(this.Config as any).CustomData['MainField'])
        if(ret) return ret.Label;
        else return (this.Config as any).CustomData['MainField']
      }
    }
    getLeftField() {
